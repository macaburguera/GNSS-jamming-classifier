...
# Folder for plots (../plots relative to BASE_DIR)
...
# ================== Pick one .mat per class for grid ==================
picked = {}
for cls in CLASSES:
    folder = BASE_DIR / cls
...
def grid_jammertest():
    ...
    out_path = PLOTS_DIR / "overview_grid.png"
    fig.savefig(out_path, dpi=200)
    print(f"[SAVE] overview grid -> {out_path}")

    plt.show()
    plt.close(fig)

# ================== Save plots for FIRST N observations ==============
def save_spectrogram_for_file(mat_path, cls_name):
    snap, Fs_snap = load_iq_and_fs(mat_path)
    f, tt, SdB, vmin, vmax = compute_spectrogram(snap, Fs_snap)
    ...
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Frequency (MHz)")
    ax.set_title(f"{cls_name} — {mat_path.stem}")
    cb = fig.colorbar(im, ax=ax)
    cb.set_label("Power (dB)")
    fig.tight_layout()

    out_name = f"{cls_name}_{mat_path.stem}.png"
    out_path = PLOTS_DIR / out_name
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    print(f"[SAVE] {out_path}")

def save_first_observations():
    for cls in CLASSES:
        folder = BASE_DIR / cls
        all_files = sorted(folder.glob("*.mat"))
        if not all_files:
            print(f"[WARN] no .mat in {folder}")
            continue

        files = all_files[:MAX_OBS_PER_CLASS]
        print(f"[CLASS] {cls}: using first {len(files)} of {len(all_files)} files")

        for i, mat_path in enumerate(files, start=1):
            print(f"  -> ({i}/{len(files)}) {mat_path.name}")
            try:
                save_spectrogram_for_file(mat_path, cls)
            except Exception as e:
                print(f"  [ERROR] {mat_path}: {e}")

# ================== Run ==================
if __name__ == "__main__":
    grid_jammertest()        # overview grid
    save_first_observations()  # only first XX observations per class
