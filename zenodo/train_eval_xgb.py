...
# ----------------------- Defaults (aligned with your other scripts) -----------------------
DEFAULT_ARTIFACTS_ROOT = Path("./artifacts")

# Simplified, readable bins (same as in train_eval_logreg.py)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train & evaluate XGBoost on cached features with per-JSR/CNR analysis."
    )
    p.add_argument(
        "--features_dir",
        type=str,
...
def latest_prep_dir(root: Path) -> Optional[Path]:
    ...
    if not root.exists():
        return None
    candidates = sorted(
        [p for p in root.iterdir() if p.is_dir() and p.name.startswith("enc_run_")], #prep_run, mr_run, change accordingly
        key=lambda p: p.name,
        reverse=True,
    )
...
def main():
    ...
    pipe = Pipeline([
        ("clf", XGBClassifier(
            objective="multi:softmax",         # predict class IDs directly
    ...
    search = GridSearchCV(
        pipe, grid, cv=ps, scoring="f1_macro", n_jobs=-1, refit=False, verbose=2
    )
    ...
    # Feature importance (gain) mapped to your feature names when possible
    try:
        booster = best.named_steps["clf"]
        fscore_gain = booster.get_booster().get_score(importance_type="gain")  # dict like {"f0": gain, ...}
        rows = []
        ...
        # Best-effort only; ignore if something changes in xgboost internals
        pass
